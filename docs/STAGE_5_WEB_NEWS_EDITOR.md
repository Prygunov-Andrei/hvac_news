...
|------|----------|-------------|--------|
| 5.1 | CRUD API для новостей | — | ⏳ Ожидает |
| 5.2 | Загрузка медиафайлов | 5.1 | ⏳ Ожидает |
| 5.3 | Интеграция LLM для перевода | 5.1 | ⏳ Ожидает |
| 5.4 | Отложенная публикация | 5.1 | ⏳ Ожидает |

**Рекомендуемый порядок:** 5.1 → 5.2 → 5.4 → 5.3

*(LLM последним, т.к. требует API ключи и настройку)*
